...
LABEL_OPTION="@pane_tabs_label"
# Rendered "[N/M] name" is mirrored into a pane option because programs inside
# the pane (fish, nvim) overwrite pane_title with OSC escapes, which silently
# wiped the status-right indicator. Pane options can't be clobbered that way.
TITLE_OPTION="@pane_tabs_title"
...
update_pane_title() {
    local pane="$1" group index count position tab_name title
    group="$(get_option "$pane" "$GROUP_OPTION")"
    if [ -z "$group" ]; then
        # No tabs — clear to default (pane_title will show shell/command)
        tmux select-pane -t "$pane" -T ""
        set_option "$pane" "$TITLE_OPTION" ""
        return
    fi
    index="$(get_option "$pane" "$INDEX_OPTION")"
    count="$(tab_count "$group")"
    position="$(tab_position "$group" "$index")"
    tab_name="$(get_option "$pane" "$LABEL_OPTION")"
    [ -n "$tab_name" ] || tab_name="$(tmux display-message -p -t "$pane" '#{pane_current_command}')"
    title="[$position/$count] $tab_name"
    tmux select-pane -t "$pane" -T "$title"
    set_option "$pane" "$TITLE_OPTION" "$title"
}

rename_tab() {
    local pane="$1" label="$2" group
    group="$(get_option "$pane" "$GROUP_OPTION")"
    if [ -z "$group" ]; then
        message "This pane has no tabs"
        return
    fi
    # An empty label clears the override and falls back to the running command.
    set_option "$pane" "$LABEL_OPTION" "$label"
    update_pane_title "$pane"
    if [ -n "$label" ]; then
        message "Renamed tab to $label"
    else
        message "Tab name reset"
    fi
}
...
case "$command" in
    ...
    rename) rename_tab "$pane" "$label" ;;
    *)
        printf 'usage: %s {new|next|prev|close|rename} pane-id [label]\n' "$0" >&2
        exit 2
        ;;
esac
